...
\begin{document}
    ...
    As for multilayer graphs \cite{mlkiv}, ${\cal L}$ is a set of $d$ {\em aspects}; ${\cal L} = \{L_i\}_{i=1}^d$, and an element $\alpha_i$ of $L_i$ is named {\em elementary layer}. An element of $L=L_1\times \dots \times L_d$ is named {\em layer}. From those elements, the set of {\em node-layers } $V_M$ is defined as follows : $V_M = \{ (u,\alpha), u\in V, \alpha \in L\}$.
